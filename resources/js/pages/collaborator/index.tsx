...
const getColumns = (
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            const handleChange = () => {
                router.put(
                    route("collaborators.active", row.original.external_id),
                    {
                        preserveState: true,
                        preserveScroll: true,
                    }
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            const handleUpdate = (
                ...
                router.put(route("collaborators.update", external_id), values, {
                    preserveState: true,
                    preserveScroll: true,
                    onSuccess: () => {
                        onAddUserForm();
            ...
            const handleDelete = (external_id: string) => {
                router.delete(route("collaborators.destroy", external_id), {
                    preserveState: true,
                    preserveScroll: true,
                });
            };
            ...
            const handleResetPassword = (external_id: string) => {
                router.put(route("collaborators.reset-password", external_id), {
                    preserveState: true,
                    preserveScroll: true,
                });
            };
                        ...
                        <div className="flex items-center justify-end gap-2">
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end">
                                    ...
                                    {useHasPermission("collaborators_list") && (
                                        <Link
                                            href={route(
                                                "collaborators.show",
                                                user.external_id
                                    ...
                                    {useHasPermission("collaborators_edit") && (
                                        <>
                                            <DropdownMenuItem
...
export default function Index({
    ...
    const [userData, setUserData] = useState<IUser[]>(collaborators.data);
    const [columns, setColumns] = useState(getColumns(rolesForCreateOptions));
    const [currentPage, setCurrentPage] = useState(collaborators.current_page);
    const [lastPage, setLastPage] = useState(collaborators.last_page);
    const [perPage, setPerPage] = useState(collaborators.per_page);
    ...
    useEffect(() => {
        setUserData(collaborators.data);
        setColumns(getColumns(rolesForCreateOptions));
        setCurrentPage(collaborators.current_page);
        setLastPage(collaborators.last_page);
        setPerPage(collaborators.per_page);
    }, [collaborators]);
    ...
    const onPageChange = (page: number) => {
        setCurrentPage(page);
        if (page != collaborators.current_page) {
    ...
    const onRowsPerPageChange = (rows: number) => {
        setPerPage(rows);
        setCurrentPage(1);
        if (rows != collaborators.per_page) {
    ...
    const handleSubmit = (
        ...
        router.post(route("collaborators.store"), values, {
            preserveState: true,
            preserveScroll: true,
            onSuccess: () => {
                onAddUserForm();
    ...
    return (
        <AuthenticatedLayout header="Lista de Colaboradores">
            <Head title="Lista de Colaboradores" />
